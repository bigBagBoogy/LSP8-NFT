...
contract LuksoCloneX is LSP8IdentifiableDigitalAsset, ReentrancyGuard {
    ...
    function privateMint(address to, uint256 amount, bool allowNonLSP1Recipient, bytes32[] calldata merkleProof)
        external
        payable
        nonReentrant
    {
        ...
        require(tokenSupply + amount <= MAX_SUPPLY, "Exceeds MAX_SUPPLY");
        require(
            _mintedTokensPerWhitelistedAddress[msg.sender] + amount <= MAX_MINT_PER_WHITELISTED_ADDRESS,
            "Exceeds MAX_MINT_PER_ADDRESS"
        );
